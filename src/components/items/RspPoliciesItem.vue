        ...
        <v-card-item class="ma-0">
          <v-row no-gutters dense justify="space-around">
            ...
            <v-col cols="2" align-self="center" class="pa-1">
              <v-btn  class="me-2 text-none" color="secondary" variant="outlined" rounded @click="handlePreview(item?.file_url)">
                Preview
              </v-btn>
              <v-btn v-if="props.actionId === 1"
...
const ActionMenus = computed({
    get() {
      switch(props.actionId){
        case 2 :return [
                { title: 'Deactivated policy', icon: 'mdi-chevron-right' , color : 'red' ,emit: 'DEACTIVATED_POLICY' },
